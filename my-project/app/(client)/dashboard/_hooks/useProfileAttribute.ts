...
import {getProfileAttribute} from "@/app/shared-hooks/getProfileAttribute";
...
export const useProfileAttribute = (key: string) => {
    const { data, loading, error, mutate } = useSWRFix<ProfileAttributeResponse>({
        ...
        fetcher: async () => {
            ...
            if (!response) {
                throw new Error(`Failed to fetch profile attribute for key: ${key}`);
            }
            return response;
        },
        transform: (data) => {
            const updatedAttr = data.options ? data.options.split(",").map(s => s.trim()).filter(Boolean) : [];
