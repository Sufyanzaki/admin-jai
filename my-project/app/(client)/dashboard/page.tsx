        ...
        return (
            <div className="flex items-center flex-col justify-center h-64 gap-3">
                <h2 className="text-2xl font-bold text-red-600">
                    {t("Error loading your profile information")}
                </h2>
                <p className="text-muted-foreground">{error.message}</p>
                <Link href={"/auth/login"}>
                    <Button variant={"theme"}>{t("Log In")}</Button>
                </Link>
            </div>
        );
    }
                    ...
                    <Card className="border-[#dee2e6] rounded-[5px] p-5 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between shadow-none">
                        <div className="flex gap-5 items-center">
                            ...
                            <div className="text-black">
                                <h5 className="text-lg font-semibold">{t("New Message")}</h5>
                                <p className="text-sm lg:text-base font-normal">
                                    {t("You got {{count}} new messages.", { count: response?.user.messageCount })}
                                </p>
                        ...
                        <div className="flex gap-3.5 justify-center">
                            <Button variant="theme" size="lg">{t("View")}</Button>
                        </div>
                    </Card>
                </div>
                    ...
                    <div className="py-6 space-y-8">
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-4 space-y-3">
                                    <h2 className="font-semibold text-xl md:text-3xl">
                                        {t("Here are Today's top Matches for you.")}
                                    </h2>
                                    <Button
                                    ...
                                    {error ? (
                                        <div className="flex items-center justify-center h-screen">
                                            <div className="text-center">
                                                <h2 className="text-2xl font-bold text-red-600">
                                                    {t("Error loading Today's Matches")}
                                                </h2>
                                                <p className="text-muted-foreground">{error.message}</p>
                                            </div>
                                        </div>
                        ...
                        <div className="mb-4 space-y-3">
                            <div className="flex items-center justify-between mb-4 space-y-3">
                                <div>
                                    <h2 className="font-semibold text-xl md:text-3xl">
                                        {t("Members you may like")}
                                    </h2>
                                    <p>{t("Members who match many of your preferences")}</p>
                                </div>
                                <Button
                                    variant="ghost"
                                    size="sm"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-6 gap-5">
                                {mayLikeError ? (
                                    <div className="flex items-center justify-center h-screen">
                                        <div className="text-center">
                                            <h2 className="text-2xl font-bold text-red-600">
                                                {t("Error loading Members You May Like")}
                                            </h2>
                                            <p className="text-muted-foreground">{mayLikeError.message}</p>
                                        </div>
                                    </div>
                                ) : mayLikeLoading ? (
                                    Array.from({ length: 6 }).map((_, index) => (
                    ...
                    <Card className="border-[#dee2e6] rounded-[5px] py-12 px-5 shadow-none">
                        <div className="flex flex-col items-center gap-6">
                            <p className="text-black text-sm lg:text-base font-normal">
                                {t("You haven't yet completed your full profile registration. Complete your profile to continue finding a partner.")}
                            </p>
