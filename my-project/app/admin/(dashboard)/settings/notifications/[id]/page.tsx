...
import { useTranslation } from "react-i18next";import { useLanguages } from "@/app/admin/(dashboard)/settings/_hooks/useLanguages";
...
export default function EditEmailTemplatePage() {
  ...
  if (loading || languagesLoading) return (
    <div className="flex items-center flex-col justify-center h-64">
      <Preloader />
      <p className="text-sm">{t('Loading Template')}</p>
    </div>
  )
  if (error) return <div className="p-6 text-red-500">{t('Failed to load template.')}</div>;
  if (!emailTemplate) return <div className="p-6 text-muted-foreground">{t('Template not found.')}</div>;
  if (!languages || languages.length < 1) return <div className="p-6 text-red-500 text-lg text-center py-12">{t('Please add a language.')}</div>;
                ...
                <form className="space-y-8" onSubmit={handleSubmit(values => onSubmit(values, () => console.log("Edit successfully")))}>
                  <div className="space-y-6 rounded-lg border p-6">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {t('Enable or disable this email template')}
                        </p>
                      </div>
                      <Controller
                        name="isActive"
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        placeholder={t('e.g., Welcome to Our Platform ({{lang}})', { lang: lang.name })}
                    ...
                    <div className="space-y-2">
                      ...
                      />
                      <p className="text-sm text-muted-foreground">
                        {t('Use the toolbar to format your email content')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-end gap-4">
                    <Button type="submit" className="px-8" disabled={isLoading}>
                      {isLoading ? t('Saving...') : t('Save {{lang}} Template', { lang: lang.name })}
                    </Button>
                  </div>
                </form>
              </CardContent>
