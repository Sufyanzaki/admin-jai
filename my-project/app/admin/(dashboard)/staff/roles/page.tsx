  ...
  if (loading) {
    return (
      <div className="flex items-center flex-col justify-center h-64">
        <Preloader />
        <p className="text-sm">{t("Loading roles...")}</p>
      </div>
    )
  }
  if (error) {
    return <div className="p-8 text-center text-red-500">{t("Failed to load roles.")}</div>;
  }
  if (!roles) {
    return <div className="p-8 text-center text-lg">{t("No roles found.")}</div>;
  }
  ...
  return (
    <div className="flex flex-col gap-6 p-4 xl:p-6">
      <div className="flex items-center gap-4 flex-wrap">
        ...
        <div>
          <h1 className="text-2xl font-bold">{t("Roles & Permissions")}</h1>
          <p className="text-sm text-muted-foreground">{t("Manage staff access and security controls")}</p>
        </div>
      </div>
      ...
      {canView ? (
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <Card className="bg-background">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t("Total Roles")}</CardTitle>
              ...
              <Dialog>
                ...
                <DialogContent className="bg-background">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      {t("Define a new role with specific permissions for staff members.")}
                    </DialogDescription>
                  </DialogHeader>
                  <RoleForm />
                </DialogContent>
        ...
        <div className="flex items-center justify-center h-64">
          <p className="text-muted-foreground">{t("You don't have permission to view roles.")}</p>
        </div>
      )}
    </div>
  )
