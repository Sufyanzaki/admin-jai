            ...
            <Popover open={open} onOpenChange={setOpen}>
                <PopoverTrigger asChild>
                    <div
                        id={inputId}
                        className={cn(
                            "flex min-h-12 w-full flex-wrap items-center gap-1 rounded-[5px] border border-app-border bg-background px-3 py-2 text-base placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 md:text-sm"
                        )}
                    >
                        {selected.length > 0 ? (
                            selected.map((item) => (
