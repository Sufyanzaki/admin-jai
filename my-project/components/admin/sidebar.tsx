...
export function Sidebar({
  ...
  useEffect(() => {
    if (isCollapsed || manuallyToggled) return;
  ...
  }, [isCollapsed, pathname, navigationItems, manuallyToggled]);
              ...
              <>
                ...
                <AnimateHeight height={openSubmenu === item.title ? "auto" : 0}>
                  <div className="ml-4 space-y-1 pl-2 pt-1">
                    {(item.submenu ?? []).map((subItem: SidebarSubItem) => (
                        <Link
                            key={subItem.title}
                            href={subItem.href}
                            className={cn(
  ...
  return (
      <TooltipProvider delayDuration={300}>
        <aside className={sidebarClasses}>
          ...
          {footerItem && (
              <div className="border-t p-2">{renderMenuItem(footerItem)}</div>
          )}
        </aside>
      </TooltipProvider>
  );
